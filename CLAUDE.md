...
- **Ordem das animações**: nada anima fora da tela. Um `IntersectionObserver` marca `#mine` e `#settle` quando aparecem, e `agenda()` põe cada bloco numa fila (Minha conta pega a vez sem esperar a chave do pix, senão o `#settle` toma a frente) — Minha conta (copiar pix brotando, depois a piscada dos ✔), então as voltas do círculo, então os riscos dos pagamentos — pra nota se preencher de cima pra baixo. As horas são absolutas e podem estar no futuro: o CSS espera com `animation-delay` positivo e retoma com negativo, então o render do poll não atrapalha. Sala ou pessoa nova chama `rearmaAnims()`, que zera tudo e observa de novo.
