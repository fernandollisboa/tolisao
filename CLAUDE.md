...
- `manifest.json` + `sw.js`: PWA mínima. O service worker é rede-primeiro com cache de reserva (só GET da própria origem); ao mudar a estratégia, troque o nome `CACHE`. O Chrome não mostra mais banner de instalar sozinho: ele só avisa a página (`beforeinstallprompt`) e quem oferece é o `#instalar`, o botão âmbar do rodapé. No iPhone esse evento não existe, então lá o botão abre um cartão ensinando Compartilhar → Adicionar à Tela de Início. Ele some quando já está instalado (`display-mode: standalone`) e quando não há o que fazer.
