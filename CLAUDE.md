...
- `fonts/`: VT323 e Permanent Marker (woff2). `diva.png` (+ `-192`, `-maskable`): ícone da PWA, favicon e a figurinha `.stain`, que é jogada sobre o código de barras quando ele entra na tela (IntersectionObserver), caindo num ponto e ângulo sorteados a cada vez; `og3.jpg`: preview do WhatsApp (`og.png`/`og2.png` são as versões antigas, PNG de 724 KB, que o WhatsApp deixou de mostrar; o scraper dele ignora imagem grande demais, então mantenha o arquivo abaixo de uns 300 KB). Trocar de nome também fura o cache do preview, que dura dias.
