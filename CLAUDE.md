...
- **Ordem das animações**: nada anima fora da tela. Um `IntersectionObserver` marca `#mine`, `#itemsSec` e `#settle` quando aparecem, e `agenda()` põe cada bloco numa fila, na ordem em que aparecem na página: Minha conta (a piscada dos ✔ primeiro, uma linha atrás da outra, e só então o copiar pix brotando de cada uma), depois os dois toquinhos no botão dos itens (`apertinho`), depois as voltas do círculo das suas linhas de acerto — uma volta fecha antes da outra abrir —, e por último os riscos dos pagamentos. Minha conta pega a vez sem esperar a chave do pix, senão o `#settle` toma a frente; e os pagamentos da carga são carimbados em bloco quando o `#settle` pega a vez, senão o primeiro render marcava todos com a hora de agora e eles riscavam antes de tudo. As horas são absolutas e podem estar no futuro: o CSS espera com `animation-delay` positivo e retoma com negativo, então o render do poll não atrapalha. Sala ou pessoa nova chama `rearmaAnims()`, que zera tudo e observa de novo.
