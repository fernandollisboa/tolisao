...
- Evento recém-criado: enquanto não houver nenhum gasto anotado e a pessoa já tiver dito quem é, o balão `#dica` aponta pro ✎.
...
- Ações ficam só em Minha conta: nas linhas de quem você deve aparecem `quitar` (✔, com sombra) e `copiar pix`. A seção Falta pagar (`#settle`) é só leitura, mostra quem paga quem. O `cobrar` segue no código atrás da constante `COBRAR`, hoje desligado. O link `desfazer` nas linhas pagas sai pela constante `DESFAZER`; existe pra testar. O jeito de dividir é a própria palavra da frase do `#splitHint`: `igualmente` é um link que vira `em partes diferentes` e volta; no caso de empréstimo a frase termina oferecendo o mesmo link. Texto do zap (`summaryText`) leva o acerto e o link do evento; a imagem (`renderReceipt`) leva itens e um saldo que é só quem paga quem mais os já quites; os nomes aparecem marcados dentro das próprias linhas, sem legenda no topo.
