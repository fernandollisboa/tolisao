...
- **Ordem das animações**: nada anima fora da tela. Um `IntersectionObserver` marca `#mine` e `#settle` quando aparecem, e `agenda()` põe cada bloco numa fila, na ordem em que aparecem na página: Minha conta (copiar pix brotando, depois a piscada dos ✔, uma linha atrás da outra), então as voltas do círculo das suas linhas de acerto, também uma atrás da outra, e por último os riscos dos pagamentos. Minha conta pega a vez sem esperar a chave do pix, senão o `#settle` toma a frente; e os pagamentos da carga são carimbados em bloco quando o `#settle` pega a vez, senão o primeiro render marcava todos com a hora de agora e eles riscavam antes de tudo. As horas são absolutas e podem estar no futuro: o CSS espera com `animation-delay` positivo e retoma com negativo, então o render do poll não atrapalha. Sala ou pessoa nova chama `rearmaAnims()`, que zera tudo e observa de novo.
