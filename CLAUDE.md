...
- `specs/`: cucumber em português (`playwright-bdd` em cima do `@playwright/test`).
  ...
  - `preview.cjs` e `video.cjs` não são testes, são geradores de imagem e de `.webm` (cenas `cascata`, `pix`, `piscas`, `troca`, `chave`, `ficha`, `pega`, `chato`, `toque`, `itens`, `abas`, `risco`; `--css=` e `--js=` pra comparar variações). `_ficha.cjs` tem os gestos da ficha.
