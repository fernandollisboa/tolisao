...
- `tests/`: scripts Playwright (`node tests/run-all.cjs`). Sem framework: cada script sobe um servidor local, intercepta o Firebase e imprime o que checou. `tests/preview.cjs` e `tests/video.cjs` não são testes: são os geradores pro usuário ver — o primeiro tira imagem (e folha comparativa de opções), o segundo grava um `.webm` de uma cena (`pix`, `piscas`, `chave`, `ficha`, `risco`; cada uma pode trazer os próprios dados), porque imagem parada não mostra animação.
