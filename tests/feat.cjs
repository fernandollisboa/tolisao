...
const srv = http.createServer((q, r) => { if (q.url.startsWith('/fonts/')) { r.setHeader('Content-Type','font/woff2'); return r.end(fs.readFileSync(path.join(ROOT, q.url))); } r.setHeader('Content-Type','text/html'); r.end(html); }).listen(4183);
...
(async () => {
  ...
  await p1.waitForSelector('#whoSel'); await p1.selectOption('#whoSel', { label: 'Júlia' }); await p1.click('#whoForm button'); await p1.waitForTimeout(300);
  ...
  await p2.goto('http://localhost:4183/#c=bailedamada'); await p2.waitForSelector('#whoSel'); await p2.selectOption('#whoSel', { label: 'Lia' }); await p2.click('#whoForm button'); await p2.waitForTimeout(400);
