...
(async () => { const b = await chromium.launch(); const c = await b.newContext({ viewport: { width: 390, height: 844 } }); const errs = [];
  ...
  await p.goto('http://localhost:4187/#c=bailedamada'); await p.click('#whoBtn'); await p.waitForSelector('#whoSel'); await p.selectOption('#whoSel', { label: 'Fernando' }); await p.click('#whoForm button'); await p.waitForTimeout(800);
